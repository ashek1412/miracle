...
class WPFTS_Index 
{
	...
	public function getDbScheme()
	{
		...
		$engine_type = 'InnoDB'; // intval($wpfts_core->get_option('is_innodb')) ? 'InnoDB' : 'MyISAM';
	...
	{	
		...
		$log_chunks = array();
		foreach ($log_chunks as $kk => $dd) {
			$log_chunks[] = ''.$kk.' => '.mb_strlen($dd);
		}
	...
	public function logtime($s)
	{
		if ($this->_is_log || $this->_is_show) {
			...
			if ($this->_is_log) {
				$log_fn = dirname(__FILE__).'/index_log.php';

				file_put_contents($log_fn, $ss, FILE_APPEND);
			}
			if ($this->_is_show) {
				echo $ss;
			}
	...
	public function indexWordData($wid)
	{
		...
		foreach ($tt as $kk => $dd) {
			...
			if (strlen($pk) > 1000 * 4) {
				...
				if (strlen($wpdb->last_error) < 1) {
					$this->logtime('set doc ids ('.$pk_size.','.strlen($pk).')');
				} else {
		...
		if (strlen($pk) > 0) {
			...
			if (strlen($wpdb->last_error) < 1) {
				$this->logtime('set remaining doc ids ('.$pk_size.','.strlen($pk).')');
			} else {
				$this->logtime('MySQL Error: '.$wpdb->last_error);
				return false;
			}
...
//print_r($sess);
	...
	{	
		...
		if ($ret['n_pending'] < 1) {
			...
			$ret['nw_act'] = $res[0]['nw_act'];
			$ret['nw_total'] = $res[0]['nw_total'];
		} else {
			$ret['nw_act'] = 0;
			$ret['nw_total'] = 0;
		}
