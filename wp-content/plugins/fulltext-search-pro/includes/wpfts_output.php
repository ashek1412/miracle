...
class WPFTS_Output
{
	...
	{	
		...
		<p class="wpfts_status">
			<?php
			if (!$status['enabled']) {
				?><span class="wpfts_status_bullet wpfts_red" title="<?php echo __('The Search Index was disabled in configuration.', 'fulltext-search'); ?>">&#9679;</span>&nbsp;<b><?php echo __('Disabled', 'fulltext-search'); ?></b><?php
			} else {
				if ($status['index_ready']) {
					?><span class="wpfts_status_bullet wpfts_green">&#9679;</span>&nbsp;<b><?php echo __('Active', 'fulltext-search'); ?></b><?php
				} else {
					?><span class="wpfts_status_bullet wpfts_yellow" title="<?php echo __('The Search Index will be activated after the indexing process is complete.', 'fulltext-search'); ?>">&#9679;</span>&nbsp;<b><?php echo __('Awaiting', 'fulltext-search'); ?></b><?php
				}
			}
			?>
		</p>
		<p><?php echo __('In Index', 'fulltext-search'); ?>: <b><span id="wpfts_st_inindex"><?php echo $status['n_inindex']; ?></span></b> <?php echo __('records', 'fulltext-search'); ?></p>
		<?php 
		if ($status['autoreindex']) {
			if (($status['n_pending'] > 0) || ($status['nw_act'] < $status['nw_total'])) {
		...
		<div class="wpfts_indexer_info">
			<?php
				if ($status['n_pending'] > 0) {
					...
					<span><?php echo __('Indexed', 'fulltext-search'); ?>: <b><span id="wpfts_st_actual"><?php echo $status['n_actual']; ?></span></b> <?php echo __('records', 'fulltext-search'); ?></span><br>
					<span><?php echo __('Pending', 'fulltext-search'); ?>: <b><span id="wpfts_st_pending"><?php echo $status['n_pending']; ?></span></b> <?php echo __('records', 'fulltext-search'); ?></span>
					<div id="wpfts_st_1">
						<p class="wpfts_st_percent"><img src="<?php echo $wpfts_core->root_url; ?>/style/waiting16.gif" alt="" title="<?php echo __('Indexing is in progress', 'fulltext-search'); ?>">&nbsp;<span><?php echo sprintf('%.2f', $percent).'%'; ?></span></p>
						<p><?php echo __('Est. time left: ', 'fulltext-search'); ?><span class="wpfts_st_esttime"><?php echo $status['est_time']; ?></span></p>
					</div>
					<?php
				} else {
					?>
					<p><span class="wpfts_indexing_status_bullet wpfts_green">&#9679;</span>&nbsp;<b><?php echo $status['n_actual']; ?></b> <?php echo __('post records done', 'fulltext-search'); ?></p>
					<?php
				}
			?>
			<?php
			if ($status['n_pending'] > 0) {
				...
				// Also check for word index
				if ($status['nw_act'] < $status['nw_total']) {
					$percent = sprintf('%.2f', $status['nw_act'] * 100 / $status['nw_total']);
					?>
					<p><span class="wpfts_indexing_status_bullet wpfts_yellow">&#9679;</span>&nbsp;<b><?php echo __('Processing word index', 'fulltext-search'); ?></b></p>
					<div class="progress" title="<?php echo $status['nw_act'].' / '.$status['nw_total']; ?>">
						<div class="progress-bar progress-bar-striped progress-bar-animated" role="progressbar" aria-valuenow="<?php echo $percent; ?>" aria-valuemin="0" aria-valuemax="100" style="width: <?php echo $percent; ?>%"><?php echo $percent; ?>%</div>
					</div>					
					<?php
				} else {
					// All words were processed
					?>
					<p><span class="wpfts_indexing_status_bullet wpfts_green">&#9679;</span>&nbsp;<b><?php echo __('Word index done', 'fulltext-search'); ?></b></p>
					<?php
				}
			}
			?>
		...
		<?php
				...
				?><p><span class="wpfts_indexing_status_bullet wpfts_green">&#9679;</span>&nbsp;<?php echo __('Index is OK', 'fulltext-search'); ?></p><?php
			}
		} else {
			?><p><span class="wpfts_indexing_status_bullet wpfts_red">&#9679;</span>&nbsp;<?php echo __('Indexing is disabled', 'fulltext-search'); ?></p><?php
		}
		?>
			</div>
		</div>
	...
	public function useful_box($post) 
	{
		...
		<div class="card mb-2">
			...
			<div class="card-body">
				...
				<p><a href="<?php echo $wpfts_core->_wpfts_domain.$wpfts_core->_documentation_link; ?>" target="_blank"><?php echo __('WPFTS Documentation', 'fulltext-search'); ?></a></p>
				<hr>
