...
class WPFTS_Core 
{
	...
	function output_admin_notice($text, $type = 'error', $n_id = '', $is_raw = false)
	{
		if ($is_raw) {
			?>
	 ...
	 */
	public function reindex_post($post_id, $is_force_remove = false, $is_attachment_cache_remove = false) 
	{
		...
		if ($post && (!$is_force_remove)) {
			...
			if (!$this->defer_indexing) {
				$chunks2 = $this->getPostChunks($post_id, $is_attachment_cache_remove);
			}
			...
			$this->removeAttachmentCache($post_id);
			return true;
		}
	}
	...
	public function removeAttachmentCache($post_id)
	{
		global $wpdb;
	...
	public function getPostChunks($post_id, $is_refresh_cache_attachments = false)
	{
		$post = get_post($post_id);
		$chunks = array();
		$tt = $post->post_title;
		...
		if (strlen($tt2) > 0) {
			if ($this->get_option('content_strip_tags') != 0) {
				$chunks['post_content'] = $this->contentStripTags($tt2);
			} else {
		...
		if ($post->post_type == 'attachment') {
			...
			$att_chunks = $this->getCachedAttachmentContent($post_id, $is_refresh_cache_attachments);
			...
			if ($this->get_option('include_attachments') != 0) {
				...
				if (count($p2) > 0) {
					...
					$sum = '';
					foreach ($p2 as $tp) {
						$extr = $this->getCachedAttachmentContent($tp->ID, $is_refresh_cache_attachments);
...
*/
				...
				$is_flush = false;
				if ((count($ids) < 1) && (microtime(true) - $start_ts < $maxtime)) {
				...
				$status['est_time'] = sprintf('%02d:%02d:%02d', $est_h, $est_m, $est_s);
		
				$this->set_option('est_time', $status['est_time']);
				
				if (($status['n_pending'] > 0) || ($is_not_acts)) {
				...
				$jx->console(sprintf(__('%s posts has been rebuilt', 'fulltext-search'), $n));
			} else {
				// Unable to index
				$jx->variable('code', 1);
			}
	...
	function index_pre_get_posts(&$wpq) 
	{
		...
		$display_attachments = intval($wpq->get('display_attachments', intval($this->get_option('display_attachments'))));
		$wpq->set('display_attachments', $display_attachments);
		...
		if (!$is_wpfts_disabled) {
			...
			if ($wpq->is_main_query() && !is_admin()) {
				...
				$wgt = isset($_GET['wpfts_wdgt']) ? $_GET['wpfts_wdgt'] : '';
				...
				// Tweak mime_types
				$t3 = trim($this->get_option('limit_mimetypes'));
				if (strlen($t3) > 0) {
					$mtp = $wpq->get('post_mime_type');
					$wpq->set('wpfts_post_mime_type', $mtp);
					$mtp = '';
					$wpq->set('post_mime_type', $mtp);
				}
			}
	...
	function index_sql_where($where, $wpq)
	{
		...
		if (!$is_disabled) {
			...
			if ($wpq->is_main_query() && !is_admin()) {
				$t3 = trim($this->get_option('limit_mimetypes'));
				if (strlen($t3) > 0) {
					$z = $wpq->get('wpfts_post_mime_type');

					$t_post_mime_types = array();
					// Add from initial parameters
					if (is_array($z)) {
						$t_post_mime_types = $z;
					} else {
						if (strlen(trim($z)) > 0) {
							$t_post_mime_types = array(trim($z));
						}
					}
