...
class Docly_Helper_Class {
    ...
    public function logo() {
        ...
        if ( $use_sticky_logo == '1' || is_404() || is_search() || is_archive() || (!isset($header_type) && is_page()) ) {
            $main_logo = $sticky_logo;
            $retina_logo = $retina_sticky_logo;
        }
        ?>
    ...
    */
    ...
    function banner_title() {
        $opt = get_option('docly_opt');
        if ( is_home() ) {
            $blog_title = !empty($opt['blog_title']) ? $opt['blog_title'] : esc_html__( 'Blog', 'docly' );
            ...
            esc_html_e( 'Search result for: “', 'docly' ); echo get_search_query().'”';
        } else {
            the_title();
        }
    }
    ...
    */
     function meta_css_render( $handle, $css_items ) {
        ...
        if ( !empty($opt['accent_solid_color_opt']) ) {
            ...
            $hover_box_shadow = Docly_helper()->hex2rgba($opt['accent_solid_color_opt'], 0.24);
            ...
            // Border color
            $btn_border_color = Docly_helper()->hex2rgba($opt['accent_solid_color_opt'], 0.3);
            $dynamic_css .= ".doc_border_btn, .doc_tag .nav-item .nav-link, .doc_rightsidebar .doc_switch input[type=checkbox], .nav_btn_two, .menu_two .nav_btn, .f_social_icon li a, .pagination .page-numbers {border-color: $btn_border_color !important;}";

            // Link bottom color
            $link_btm_color = Docly_helper()->hex2rgba($opt['accent_solid_color_opt'], 0.25);
            ...
            $alpha_8_bg = Docly_helper()->hex2rgba($opt['accent_solid_color_opt'], 0.8);
